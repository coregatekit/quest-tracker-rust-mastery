    ...
    T2: GuildCommandersRepository + Send + Sync,
{
    ...
    pub async fn adventurers_login(&self, login_model: LoginModel) -> Result<Passport> {
        ...
        let login_password = login_model.password.clone();
        ...
        let access_token =
            jwt_authentication::generate_jwt(secret_env.secret, &access_token_claims)?;
        let refresh_token =
            jwt_authentication::generate_jwt(secret_env.refresh_secret, &refresh_token_claims)?;
